...
$guiExe         = Join-Path $clientDir "Sm_AiCoderClient.exe"
...
if (-not (Test-ServerHealthy)) {
    Write-Host "서버가 실행 중이 아닙니다. 백그라운드에서 서버를 시작합니다..."
    Start-Process -FilePath $venv -ArgumentList "-X utf8 `"$serverLauncher`"" `
